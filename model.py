...
def main():
    ...
    adam_learning_curve = linear.train(
        X, y, optimizer=Optimizer.Adam, epochs=EPOCHS, lr=0.001
    )
    gd_learning_curve = linear.train(
        X, y, optimizer=Optimizer.GradientDescent, epochs=EPOCHS, lr=0.001
    )
    sgd_learning_curve = linear.train(
        X, y, optimizer=Optimizer.StochasticGradientDescent, epochs=EPOCHS, lr=0.001
    )

    fig = go.Figure()
    fig.add_scatter(
        x=list(range(EPOCHS)), y=adam_learning_curve, name="Adam", mode="lines"
    )
    fig.add_scatter(
        x=list(range(EPOCHS)),
        y=gd_learning_curve,
    ...
    adam_learning_curve = logistic.train(
        X, y, optimizer=Optimizer.Adam, epochs=EPOCHS, lr=0.001
    )
    gd_learning_curve = logistic.train(
        X, y, optimizer=Optimizer.GradientDescent, epochs=EPOCHS, lr=0.001
    )
    sgd_learning_curve = logistic.train(
        X, y, optimizer=Optimizer.StochasticGradientDescent, epochs=EPOCHS, lr=0.001
    )
...
def visualize_training_path(lg):
    ...
    data = pd.DataFrame.from_dict(
        {"w": weight_history, "epoch": list(range(0, len(weight_history)))}
    )
...
class LogisticRegression:
    def __init__(self, feature_count):
        self._sigmoid_vectorized = np.vectorize(
            lambda z: self._sigmoid_overflow_proof(z)
        )
...
def _optimize_adam(X, y, decay, epochs, lr, model):
    ...
    Parameters:
        X (np.array): training data features.
        y (np.array): training data target.
        decay (tuple): decay rate for first and second momentum vectors.
        epochs (int): number of iterations to train for.
    ...
    while epoch < epochs:
        ...
        model.weights = model.weights - lr * moment_hat_m / (
            np.sqrt(moment_hat_v) + epsilon
        )
